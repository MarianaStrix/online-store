...
4. Настраиваем установленные пакеты:
    ...
    # Создаем конфигурационный файл сайта (под root)
...
завершение каждого теста отменяется транзакция, начатая в его начале. По завершении выполнения всех тестов база удаляется.
...
3. В инструкции не указано как настраивался HTTPS, так как это уже выходит за рамки задания. 
Но, все равно отмечу, что сертификат используется от LetsEncrypt, а настройка производилась с помощью [acme.sh](https://github.com/Neilpang/acme.sh)